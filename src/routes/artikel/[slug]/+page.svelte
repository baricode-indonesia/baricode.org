...
<article class="mx-auto w-full max-w-3xl px-6 py-16 lg:max-w-[95%]">
	...
	{#if post.tags && post.tags.length > 0}
		<div class="mt-10 flex flex-wrap gap-2">
			{#each post.tags as tag}
				<span
					class="rounded-full bg-slate-200 px-3 py-1 text-xs font-medium text-slate-800 dark:bg-white/10 dark:text-red-100"
				>
					#{tag.name}
				</span>
			{/each}
	...
	>
		...
		<button
			type="button"
			onclick={copyLink}
			class="cursor-pointer hover:text-slate-900 dark:hover:text-white"
		>
			{#if !copiedLink}
